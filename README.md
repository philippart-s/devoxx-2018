...
A méditer en ces jours de création de nouvelle plateforme batch java réactif afin de ne peut être pas tout faire avec une seule approche (SpringBatch par exemple) mais choisir la bonne solution à utiliser pour le data streaming et la programmation réactive semble être intéressante à au moins étudier ...
...
L'architecte logiciel peut aussi être un rôle au sein du projet pour une application mais il est plus simple et quasiment obligatoire d'avoir une ligne directrice dans une grande entreprise pour le choix des technologies afin de garantir que la connaissance ne se perd pas dans une multiplication de choix technologique (maintenance, exploitabilité, ...).
Ensuite l'importance des contrats et de l'urbanisation du SI, une fois qu'un service est appelé c'est considéré comme immutable et il faut accepter qu'il fait partie du patrimoine et le faire évoluer sans jamais rien casser (on ajoute on ne supprime pas).
...
Il y a clairement des vrais retours positifs (humain **ET** business) et ne pas y aller semble être le meilleur moyen d'échouer dans les futurs projets et la garantie de ne pas être en capacité de suivre les évolutions techniques.
...
Une autre chose que j'ai trouvé intéressant est que plutôt que de mettre en place une documentation figée et qui peine à évoluer dans le temps privilégier une zone d'échanges avec questions / réponses à la stack over flow, on ne peut pas dire que ce principe ne fonctionne pas ...  
Dans le même esprit : proposer de fonctionner en interne comme le font les projets open source : lorsqu'une équipe développe quelque chose qui pourrait servir à d'autres (par exemple un loogger) externaliser ce projet dans un repo git et le proposer en utilisation comme un projet open source (issues, participation au développement, ...).
...
Et j'avais raison !  
Mais honnêtement ça valait le coup car c'est quelque chose à surveiller de très et on sent que la programmation réactive avec la programmation fonctionnelle sont deux mouvements qui vont clairement faire évoluer la façon dont nous développons nos applications.
...
Ce que j'ai pu retenir de ce REX c'est qu'une application monolithe n'est pas forcément mal au moment où elle et fait c'est ne rien faire qui est mal. Malgré cela le découpage en micros services n'est aisé et il n'existe pas de recette magique, même si l'approche DDD semble aider dans cette démarche.
...
Il faut aussi tout faire pour qu'un développeur puisse travailler dans les meilleurs conditions et sans lui faire changer de contexte constamment, le temps perdu à de telles choses est au final de l'argent de perdu car ce sont les applications qui sont créées moins vite.

La révolution numérique (et non digitale) doit venir de l'IT et des développeurs ce sont eux qui ont la connaissance technique et qui peuvent apporter de la plus-value mais cela signifie que aussi que juste faire le job ne suffit pas il faut le faire en se dépassant : *"A grands pouvoirs grandes responsabilités !"*.
...
En résumé sur ces trois jours :
 - l'agilité c'est bien mais il faut pouvoir le faire dans de bonnes conditions matérielles (espaces dédiés, ...) et humaine (équipe dédiée, PO, SM, ...) et ne pas aller jusqu'au devops lorsque l'on veut gérer un projet en utilisant une approche agile c'est se priver de tout un pan de cette philosophie !
